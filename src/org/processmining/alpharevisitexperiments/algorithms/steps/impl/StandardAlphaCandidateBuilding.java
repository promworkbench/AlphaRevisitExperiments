...
public class StandardAlphaCandidateBuilding extends CandidateBuildingStep {
    public final static String NAME = "Alpha 1.0 Candidate Building";
    final ExperimentOption[] options = {
    };
    private Set<Pair<String, String>> dfRelation;

    public StandardAlphaCandidateBuilding() {
        super(NAME);
        setOptions(options);
    }
    ...
    public Set<Pair<Set<String>, Set<String>>> buildCandidates(UIPluginContext context, LogProcessor logProcessor) {
        ...
        dfRelation = dfg.keySet().stream().filter((e) -> dfg.get(e) > 0 && activities.contains(e.getFirst()) && activities.contains(e.getSecond())).collect(Collectors.toSet());
